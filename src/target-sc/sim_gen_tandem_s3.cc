...
void IlaSim::sim_gen_tandem_s3() {
  ...
  header_ << header_indent_ << "uint32_t " << model_ptr_->name().str() << "_pc;"
          << std::endl;
  header_ << header_indent_ << model_ptr_->name().str() << "(uint32_t mode);"
          << std::endl;
...
void IlaSim::create_tandem_checkpoint(std::stringstream& tandem_check,
                                      std::string& indent) {
  auto rtl_map = load_json(tandem_rtl_);
  auto rtl_name = rtl_map["checkpoint"];
  header_ << header_indent_ << "void tandem_checkpoint(" << kRTLSimType
          << "* v);" << std::endl;
  tandem_check << indent << "void " << model_ptr_->name().str()
               << "::tandem_checkpoint(" << kRTLSimType << "* v) {"
               << std::endl;
  ...
  tandem_check
      << indent
      << "if (instr_ctr % checkpoint_period == 0) { check_all_state(v); }"
      << std::endl;
  ...
  increase_indent(indent);
  tandem_check << indent
               << "if (instr_ctr == checkpoint_seq[checkpoint_ptr]) { "
                  "check_all_state(v); checkpoint_ptr++; }"
               << std::endl;
  tandem_check << indent << "break;" << std::endl;
  decrease_indent(indent);
  ...
  increase_indent(indent);
  tandem_check << indent
               << "if (checkpoint_condition()) { check_all_state(v); }"
               << std::endl;
  tandem_check << indent << "break;" << std::endl;
  decrease_indent(indent);
...
void IlaSim::create_ilated_class_header_s3(std::stringstream& ila_wrapper,
                                           std::string& indent) {
...
void IlaSim::create_ilated_class_s3(std::stringstream& ila_wrapper,
                                    std::string& indent) {
