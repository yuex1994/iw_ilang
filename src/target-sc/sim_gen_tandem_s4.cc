...
void IlaSim::sim_gen_tandem_s4() {
  ...
  header_ << header_indent_ << "void tandem_swap(" << kRTLSimType << " *v);"
          << std::endl;
...
void IlaSim::create_tandem_check_s4() {
  ...
  tandem_swap.str("");
  std::string indent = "";
  tandem_swap << indent << "#include \"" << model_ptr_->name().str() << ".h\""
              << std::endl;
...
void IlaSim::create_swap_state(std::stringstream& tandem_swap,
                               std::string& indent) {
  ...
  for (uint i = 0; i < model_ptr_->state_num(); i++) {
    ...
    try {
      ...
      if (GetUidSort(model_ptr_->state(i)->sort()) == AST_UID_SORT::MEM) {
        ...
        tandem_swap << indent << "for (int i = 0; i < " << mem_size
                    << "; i++) {" << std::endl;
        ...
        tandem_swap << indent << "v->v_top->"
                    << boost::replace_all_copy(
                           ...
                           "\"", "")
                    << "[i] = " << model_ptr_->name().str() << "_" << state_name
                    << "[i];" << std::endl;
        ...
        tandem_swap << indent << "v->v_top->"
                    << boost::replace_all_copy(
                           boost::replace_all_copy(v_name.dump(), ".", "->"),
                           "\"", "")
                    << " = " << model_ptr_->name().str() << "_" << state_name
                    << ";" << std::endl;
        ...
        tandem_swap << indent << "v->v_top->"
                    << boost::replace_all_copy(
                           boost::replace_all_copy(v_name.dump(), ".", "->"),
                           "\"", "")
                    << " = (uint32_t)" << model_ptr_->name().str() << "_"
                    << state_name << ";" << std::endl;
...
void IlaSim::create_tandem_swap(std::stringstream& tandem_swap,
                                std::string& indent) {
  tandem_swap << indent << "void " << model_ptr_->name().str()
              << "::tandem_swap(" << kRTLSimType << " *v) {" << std::endl;
  increase_indent(indent);
